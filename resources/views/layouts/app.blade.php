<!DOCTYPE html>
<html lang="tr" x-data="{ darkMode: localStorage.getItem('darkMode') === 'true', sidebarOpen: true, searchOpen: false, notificationsOpen: false, searchQuery: '', searchResults: [], searchLoading: false }" :class="{ 'dark': darkMode }">
<head>
    <meta charset="UTF-8">
...
<body class="bg-slate-100 dark:bg-slate-950 min-h-screen">
            ...
            <div class="px-4 mb-4">
                <div class="relative">
                    <input type="text" placeholder="Hızlı arama..." 
                           ...
                           readonly
                           class="w-full bg-white/10 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white placeholder-violet-300 focus:outline-none focus:ring-2 focus:ring-violet-400/50 focus:border-transparent cursor-pointer">
            ...
            <header class="sticky top-0 z-40 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border-b border-slate-200 dark:border-slate-800">
                <div class="flex items-center justify-between px-6 py-4">
                    ...
                    <div class="flex items-center space-x-3">
                        ...
                        <!-- Notifications -->
                        <div class="relative" x-data="{ recentActivities: [] }" x-init="fetch('/api/recent-activities').then(r => r.json()).then(d => recentActivities = d).catch(() => recentActivities = [])">
                            <button @click="notificationsOpen = !notificationsOpen" class="relative p-2.5 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-700 transition-all duration-200">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"/>
                                </svg>
                                <span x-show="recentActivities.length > 0" class="absolute top-1.5 right-1.5 w-2 h-2 bg-rose-500 rounded-full"></span>
                            </button>
                            ...
                            <div x-show="notificationsOpen" 
                                 ...
                                 class="absolute right-0 mt-2 w-80 bg-white dark:bg-slate-800 rounded-xl shadow-xl border border-slate-200 dark:border-slate-700 z-50"
                                 x-cloak>
                                <div class="p-4 border-b border-slate-200 dark:border-slate-700">
                                    <h3 class="font-semibold text-slate-800 dark:text-white">Bildirimler</h3>
                                </div>
                                <div class="max-h-64 overflow-y-auto">
                                    ...
                                    <template x-for="activity in recentActivities" :key="activity.id">
                                        <a :href="'/companies/' + activity.company_id" class="block px-4 py-3 hover:bg-slate-50 dark:hover:bg-slate-700 border-b border-slate-100 dark:border-slate-700 last:border-0">
                                            <p class="text-sm font-medium text-slate-800 dark:text-white" x-text="activity.description"></p>
                                            <p class="text-xs text-slate-500 dark:text-slate-400 mt-1" x-text="activity.created_at"></p>
                                        </a>
                                    </template>
                                </div>
                                <div class="p-3 border-t border-slate-200 dark:border-slate-700">
                                    <a href="{{ route('activities.index') }}" class="block text-center text-sm text-violet-600 dark:text-violet-400 hover:underline">Tüm aktiviteleri gör</a>
                                </div>
                            </div>
                        </div>
    ...
    <div x-show="searchOpen" 
         ...
         x-cloak>
        <div class="min-h-screen px-4 pt-20 text-center">
            <div class="fixed inset-0 bg-black/50 backdrop-blur-sm" @click="searchOpen = false"></div>
            
            <div class="inline-block w-full max-w-xl p-0 overflow-hidden text-left align-top transition-all transform bg-white dark:bg-slate-800 shadow-2xl rounded-2xl">
                <div class="relative">
                    ...
                    <input type="text" 
                           ...
                           @input.debounce.300ms="
                               if(searchQuery.length >= 2) {
                                   ...
                                   fetch('/api/companies/search?q=' + encodeURIComponent(searchQuery))
                                       .then(r => r.json())
                                       .then(d => { searchResults = d; searchLoading = false; })
                                       .catch(() => { searchResults = []; searchLoading = false; });
                               } else {
                                   searchResults = [];
                               }
                           "
                           ...
                           class="w-full pl-12 pr-4 py-4 text-lg border-0 bg-transparent text-slate-800 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-0">
                    <div class="absolute right-4 top-1/2 -translate-y-1/2">
                        <kbd @click="searchOpen = false" class="px-2 py-1 text-xs bg-slate-100 dark:bg-slate-700 text-slate-500 dark:text-slate-400 rounded cursor-pointer hover:bg-slate-200 dark:hover:bg-slate-600">ESC</kbd>
                    </div>
                </div>
                
                <div x-show="searchLoading" class="px-4 py-6 text-center border-t border-slate-200 dark:border-slate-700">
                    <svg class="animate-spin h-6 w-6 mx-auto text-violet-600" fill="none" viewBox="0 0 24 24">
                        <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                        <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                    </svg>
                </div>
                
                <div x-show="!searchLoading && searchResults.length > 0" class="border-t border-slate-200 dark:border-slate-700 max-h-80 overflow-y-auto">
                    <template x-for="company in searchResults" :key="company.id">
                        <a :href="'/companies/' + company.id" 
                           @click="searchOpen = false"
                           class="flex items-center px-4 py-3 hover:bg-slate-50 dark:hover:bg-slate-700 border-b border-slate-100 dark:border-slate-700 last:border-0">
                            <div class="w-10 h-10 rounded-lg bg-gradient-to-br from-blue-400 to-indigo-500 flex items-center justify-center text-white font-semibold mr-3" x-text="company.name.charAt(0).toUpperCase()"></div>
                            <div>
                                <p class="font-medium text-slate-800 dark:text-white" x-text="company.name"></p>
                                <p class="text-sm text-slate-500 dark:text-slate-400" x-text="company.address || 'Adres belirtilmemiş'"></p>
                            </div>
                        </a>
                    </template>
                </div>
                
                <div x-show="!searchLoading && searchQuery.length >= 2 && searchResults.length === 0" class="px-4 py-6 text-center border-t border-slate-200 dark:border-slate-700">
                    <p class="text-slate-500 dark:text-slate-400">Sonuç bulunamadı</p>
                </div>
                
                <div x-show="!searchLoading && searchQuery.length < 2" class="px-4 py-4 text-center border-t border-slate-200 dark:border-slate-700">
                    <p class="text-sm text-slate-500 dark:text-slate-400">En az 2 karakter girin</p>
                </div>
            </div>
        </div>
    </div>
